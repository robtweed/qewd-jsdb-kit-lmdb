...
use with the Lightning Memory-Mapped Database (LMDB), though the APIs described in this document work identically with all other databases
...
instantiate a QEWD-JSDB *Document Node Object* for a Document (ie a LMDB *Global*) named *jsdbDom*,
...
At this point, if you look in your LMDB database using the [*global_list*](./REPL.md#the-global_list-script) script, 
...
If you look in the LMDB Global that represents our DOM, you'll see how and where these
...
If you look in your LMDB database using the [*global_list*](./REPL.md#the-global_list-script) script you'll see what's been created:
...
As you've seen when you looked in LMDB, the QEWD-JSdb DOM is persistent, being stored
...
So you've now disconnected from LMDB.
...
methods directly access the LMDB database when they're invoked, so they always return what's
...
as a Proxy Object which accesses the LMDB data every time one of its properties or methods is
...
If you look in your LMDB database using the [*global_list*](./REPL.md#the-global_list-script) script, you'll see that the removed Nodes are still in the Global - they've just
...
QEWD-JSdb storage (ie from the LMDB Global):

        parTag.removeChild(imTag, true)
        console.log(doc.dom.output(2));

If you look in your LMDB database using the [*global_list*](./REPL.md#the-global_list-script) script at the *jsdbDom* Global, you'll see that the detached Nodes have all been
...
parsed XML as a persistent DOM in LMDB.
...
You'll find an 
[example XML document in this repository](https://github.com/robtweed/qewd-jsdb-kit-lmdb/blob/master/example.xml).  Download it and put it into your QEWD system's
installation directory:

- If you're using a Native installation of QEWD and LMDB:
...
Take a look in your LMDB database using the [*global_list*](./REPL.md#the-global_list-script) script and you'll find this DOM's data in a Global named *exampleDOM*
...
Take a look in your LMDB database using the [*global_list*](./REPL.md#the-global_list-script) script and you'll find this DOM's data in a Global named *exampleDOM2*
...
actual DOM data it is handling is actually stored in your LMDB database!
...
a look at the 
[list of examples]((https://github.com/robtweed/qewd-jsdb-kit-lmdb/blob/master/xpath_query_examples.txt)
...
Now take a look in your LMDB database using the [*global_list*](./REPL.md#the-global_list-script) script at the *userDataDom* Global that is storing the DOM's data.
...
*getDocument()* methods to save JSON to a LMDB Global and retrieve it again.  Why, then,
...
use much less LMDB storage than the DOM for representing the JSON.
...
Storage in LMDB.  For XML Documents, its benefits are clear.  However, the trick is to

...
which you'll be able to try out its abstraction of your LMDB database for yourself.
...
So, the interesting feature of implementing Global Storage within Redis, LMDB or BDB is that it opens up a huge new range of database capabilities beyond what is traditionally assumed possible in either database!
...
the QEWD-JSdb abstraction of your connected LMDB database, interactively from within the
...
You now have a connection set up with LMDB, using the *mg-dbx-bdb* interface.
...
start interactively accessing your LMDB database via the *QEWD-JSdb* APIs.
...
## Check What's Happened in LMDB

It's not very straightforward to see the low-level effects of QEWD-JSdb within your LMDB database.  You may have some success using an LMDB database viewer utility.

However, the QEWD installation kit you cloned includes a handy Node.js script file that will display the data within LMDB more meaningfully as if it was a set of Global Storage Nodes (aka Global Nodes):
...
In LMDB you'll see this has been created:
...
Take a look at the *demo* Global in LMDB using the *global_list* script and you'll see how it's stored the array:
...
Run the *global_list* script to view the Global in LMDB and you'll see:
...
Run the *global_list* script and you'll see this in LMDB, saved as:
...
Look in LMDB using the *global_list* script and you'll see that this has resulted in these records being added to the database:
...
within LMDB has sorted the index in alphabetic order for us!  This is extremely useful for indices, as you'll see.
...
Check in LMDB using the *global_list* script.  You should see that it's saved it as:
...
Look in LMDB using the *global_list* script and you'll see that an *id* of 2 was assigned to this, and the *next_id*
...
If you look in LMDB using the *global_list* script, you should see that the data record has correctly changed:
...
you can use with your LMDB database:
...
one database model.  It's a fully-functional multi-model database abstraction of LMDB.
